...
- ROS/OOS 模板已负责通用部署编排。`applicationStart` 只包含当前项目实际需要的依赖安装、构建和启动命令；除非 README 明确要求，否则不要生成系统运行时安装、系统包管理器探测、旧 PID 清理或部署重试。服务启动后必须执行有上限的本机 HTTP 就绪检查。
...
- 长期服务必须使用 `cd /root/code_deploy_application && <安装命令> && <构建命令> && { <可选环境变量> nohup <启动命令> >> /root/application.log 2>&1 & APP_PID=$!; echo "$APP_PID" > /root/application.pid; sleep 2; kill -0 "$APP_PID"; } && <HTTP 就绪检查>` 结构，按项目实际需要省略不存在的安装、构建或环境变量步骤；需要 `PORT` 时把环境变量放在 `nohup` 前，禁止写成 `nohup PORT=8080 node ...`。`&` 只能后台化花括号内的服务命令，启动命令自身不得再次守护化、fork 到后台或使用 daemon 模式；禁止把进入目录、安装或构建所在的整个 `&&` 链后台化。`$!` 必须记录实际服务进程 PID。一次 `kill -0` 只能发现入口立即退出，不能作为部署成功证据；随后必须在 60 秒内有上限地轮询 `http://127.0.0.1:<port>/`，仅 HTTP 2xx/3xx 算就绪，进程退出、HTTP 4xx/5xx 或超时必须让 Hook 非零退出。`&` 本身已是命令分隔符，禁止写成 `&;`。
